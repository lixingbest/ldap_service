...
namespace WebApplication1
{
    public partial class logontypetemp : CupaaLoginUserControl
    {
        ...
        // todo: 需要配置在配置文件中的三个常量
        const string ldapUrl = "http://10.98.4.231:8080/service/public_service/user/validate";
        ...
        /// 进行表单账号和密码验证
        /// </summary>
        /// <param name="strLogonName">登录账号</param>
        ...
        private static bool DoFormAuthentication(string strLogonName, string strPassword,out string strErrorMessage)
        {
            ...
            strErrorMessage += "开始校验用户信息";

            strErrorMessage += "用户名：" + strLogonName;
            if (strLogonName == null || strLogonName.Length == 0) {
                strErrorMessage += "用户名不能为空！";
                return isPass;
            }

            strErrorMessage += "密码：" + strPassword;
            ...
            try{
                ...
                strErrorMessage += "开始执行发送请求语句块"；

                IDictionary<string, string> parameters = new Dictionary<string, string>();
                strErrorMessage += "strLogonName UrlEncode处理前=" + strLogonName;
                strErrorMessage += "strLogonName UrlEncode处理后=" + HttpUtility.UrlEncode(strLogonName)；
                parameters.Add("user", HttpUtility.UrlEncode(strLogonName));
                strErrorMessage += "strPassword明文=" + strPassword;
                strErrorMessage += "strPassword密文=" + EncryptString(strPassword)；
                strErrorMessage += "密码UrlEncode处理后=" + HttpUtility.UrlEncode(EncryptString(strPassword))；
                parameters.Add("password", HttpUtility.UrlEncode(EncryptString(strPassword)));
                strErrorMessage += "url=" + ConfigurationManager.AppSettings["ldapUrl"];
                string url = ConfigurationManager.AppSettings["ldapUrl"];
                // 发送请求
                strErrorMessage += "开始发送请求";
                ...
                {
                    // 解析请求
                    strErrorMessage += "开始解析请求结果";
                    string mes = GetResponseString(res);
                    strErrorMessage += "请求结果内容：" + mes;
